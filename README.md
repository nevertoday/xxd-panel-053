...
<table>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2091521812878459186"><img src="./assets/examples/sample-01.jpg" alt="XXD Panel 053 样张 1"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2091521812878459186"><img src="./assets/examples/sample-02.jpg" alt="XXD Panel 053 样张 2"></a></td>
  </tr>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2091521812878459186"><img src="./assets/examples/sample-03.jpg" alt="XXD Panel 053 样张 3"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2091521812878459186"><img src="./assets/examples/sample-04.jpg" alt="XXD Panel 053 样张 4"></a></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 053 样张 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 053 样张 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 053 样张 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 053 样张 8"></td>
  </tr>
</table>

## 3:4 上下双联样张

以下四张复用上方 16:9 样张对应的四张独立素材，重新生成完整 3:4 上下双联画布；上部保留现实摄影，下部遵循 053 原始提示词重构。英文配字只从当前照片的内容、情绪或隐喻中生成，并已逐张复核。

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 053 新增上下样张 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 053 新增上下样张 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 053 新增上下样张 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 053 新增上下样张 4"></td>
  </tr>
</table>
...
**Panel 053** 适合这类任务：需要保留原图核心内容，同时希望最终视觉达到以下方向：**让被观察的线条在最恰当的位置停下**。它先从用途与期望结果出发，再决定构图、尺寸、文字和具体输出形式。
...
> 用 XXD Panel 053 帮我处理这张图，先推荐最合适的构图和尺寸。
...
| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 让被观察的线条在最恰当的位置停下 |
| **一眼可辨的特点** | 观察线稿 · 透明淡彩 · 音乐性节奏 · 近白纸面 · 大胆留白 |
...
```text
锁定身份、比例、姿态、方向与叙事关系 → 保留三个线索 → 选择值得记忆的线 → 建立一个视觉动机 → 重复、变奏、重音、停顿并停笔 → 局部加入两至四种源图淡彩 → 让近白纸面成为休止 → 让文案成为轻微回声
```

如果换成无关照片后，视觉动机、识别线索、线条节奏、色彩笔记、留白与文案不发生实质变化，结果就不属于这套 Panel。

## 成品中最容易识别的风格特点

- 至少保留三个源图专属的剪影、比例、姿态、动作、方向、结构或关系线索。
- 使用纤细、松动、不均匀的钢笔线，允许寻找线、修正、突然加重与未完成边缘；关键比例必须准确。
- 以一个轮廓或动作为视觉动机，组织重复、变奏、重音与停顿，让大片空白成为主动休止。
- 只用两至四种源图色作透明局部点染；纸色接近白色，并按源图冷暖轻微调整。
- “自然衔接”是固定中线附近的视觉过渡语言，不改变双联严格 50/50 的几何分割。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-053-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 053 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
```text
/xxd-panel-053 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-053 photo.jpg --prefs last
/xxd-panel-053 photo.jpg --prefs edit
/xxd-panel-053 photo.jpg --prefs new
```
...
/xxd-panel-053 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN

# 左右对照：3:2 横版，无文字
/xxd-panel-053 photo.jpg --mode left-right --size 3:2 --text none

# 只要设计图：9:16 手机竖版，日语文字
/xxd-panel-053 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# 四端连贯壁纸：先建立一张定调图，再按设备分别重构
/xxd-panel-053 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# 四张独立壁纸：同时指定四台设备的准确分辨率
/xxd-panel-053 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# 跟随原图比例
/xxd-panel-053 photo.jpg --mode design-only --size source --text none

# 同一用途生成方形、竖版和横版三种完整构图
/xxd-panel-053 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# 自定义比例与准确像素可以一起多选
/xxd-panel-053 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# 模型依照原始提示词决定文字，只限定语言
/xxd-panel-053 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-053 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-053 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 053；公共设置只解析一次
/xxd-panel-053 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN

# 重复参数也会累积，并把本次任务放到指定根目录
/xxd-panel-053 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
```bash
npx skills add https://github.com/nevertoday/xxd-panel-053 --skill xxd-panel-053
```

该命令会从 GitHub 获取仓库，并把同名 Skill 安装到当前 Agent。若要安装到用户级 Codex Skills 目录，可在命令末尾加上 `--global --agent codex --yes`。

Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-053`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-053-prompt.en.md) · [中文运行适配器](references/xxd-panel-053-prompt.zh-CN.md)
...
<div align="center"><strong>不是把照片画完，而是在最恰当的位置停下。</strong></div>
